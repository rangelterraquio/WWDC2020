    ...
    func addNodes(from level: Level) -> Void {
        ...
        case .level1:
             let cbl = CBLNode(texture: SKTexture(imageNamed: "cbl"), color: .blue, size: CGSize(width: 195, height: 71))
              ...
              self.addChild(cbl)
        case .level2:
            let swift = CodeCollectable(texture: SKTexture(imageNamed: "swift"), color: .clear, size: CGSize(width: 100, height: 45))
            swift.position = CGPoint(x: 686.734, y: -86.12)
            swift.physicsBody = SKPhysicsBody(rectangleOf: CGSize(width: 100, height: 45))
